...
Este enfoque mediante cavidades no solo busca explorar una nueva forma de construir mallas, sino también comparar su rendimiento con métodos existentes. En particular, se contrastará esta técnica con el generador de mallas _Polylla_ @PolyllaPaper, un sistema moderno que emplea estructuras de datos avanzadas para lograr eficiencia y calidad en la generación de polígonos.
...
#figure(
    ...
    caption: [a) Triangulación de Delaunay y Diagrama de Voronoi superpuesto,\ b) Triangulación de Delaunay restringida,\ c) Diagrama de Voronoi restringido @PolyllaPaper]
) <VoronoiExample>
...
== Estado del arte: herramientas para la generación de mallas
...
=== Triangle
El algoritmo generador de mallas poligonales implementado en Triangle @TrianglePaper se basa en el algoritmo de Ruppert @RuppertPaper y consta de 4 etapas, de las cuales las primeras 2 son exactamente las mismas que las de Ruppert. Estas involucran triangular la geometría original (@PSLGGuitarra), considerando solo los vértices dentro de su cápsula convexa, tal como se muestra en la @PSLGGuitarraTriangulada y posteriormente insertar los segmentos originales de la triangulación como se ve en la @PSLGGuitarraConstrained.
...
La tercera etapa difiere del algoritmo de Ruppert y consiste en remover los triángulos extra que están fuera del borde definido por la geometría original, como aquellos presentes en zonas que originalmente eran no convexas, a las cuales Shewchuk llama concavidades, y los presentes en agujeros (como los del interior del cuerpo de la guitarra en el ejemplo). Esto se ilustra en la @PSLGGuitarraDeleted.

#figure(
    image("/imagenes/pslgdeletedextra.png", width: 80%),
    caption: [Triangulación restringida del PSLG de la @PSLGGuitarra con triángulos extra removidos @TrianglePaper]
) <PSLGGuitarraDeleted>
...
El algoritmo Polylla destaca por sobre otros algoritmos debido a su gran simplicidad y eficiencia en comparación a algoritmos convencionales de construcción de diagramas de Voronoi restringidos, ya que toma bastante menos tiempo en construir con una cantidad de polígonos 3 veces menor y manteniendo la misma cantidad de vértices que el diagrama de Voronoi hecho a partir de la misma triangulación@PolyllaPaper. A esto se le suma también la utilidad de las mallas Polylla en simulaciones de diversos fenómenos que hacen uso del VEM@VEM mencionado anteriormente para encontrar una solución numérica. 
